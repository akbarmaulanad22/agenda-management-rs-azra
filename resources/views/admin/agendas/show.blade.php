<x-app-layout>
    ...
    <div class="space-y-6" x-data="{ showSignatureModal: false, signatureName: '', signatureUrl: '', showImageModal: false, imageUrl: '', currentImageIndex: 0 }">
            ...
            <div class="p-8">
                <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-6">
                    ...
                    <div class="flex items-center gap-2 flex-shrink-0">
                        @if($agenda->status === 'active')
                            ...
                            <a href="{{ route('agenda.input', $agenda) }}" target="_blank" class="inline-flex items-center gap-2 px-4 py-2.5 rounded-2xl bg-secondary text-white text-sm font-bold shadow-md shadow-secondary/20 hover:bg-secondary-700 active:scale-[0.98] transition-all duration-200">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931z"/></svg>
                                Link Input Publik
                            </a>
                        @endif
                        <a href="{{ route('admin.agendas.export-pdf', $agenda) }}" class="inline-flex items-center gap-2 px-4 py-2.5 rounded-2xl bg-amber-500 text-white text-sm font-bold shadow-md shadow-amber-500/20 hover:bg-amber-600 active:scale-[0.98] transition-all duration-200">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5M16.5 12L12 16.5m0 0L7.5 12m4.5 4.5V3"/></svg>
                            Export PDF
                        </a>
                        <a href="{{ route('admin.agendas.edit', $agenda) }}" class="inline-flex items-center gap-2 px-4 py-2.5 rounded-2xl bg-gray-100 text-gray-700 text-sm font-bold hover:bg-gray-200 active:scale-[0.98] transition-all duration-200">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931z"/></svg>
                            Edit
                        </a>
                    </div>
                ...
                <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
                    ...
                    <div class="p-4 rounded-2xl bg-gray-50 border border-gray-100">
                        ...
                        <p class="text-sm font-bold text-gray-800">{{ $agenda->organizer }}</p>
                    </div>
                    <div class="p-4 rounded-2xl bg-gray-50 border border-gray-100">
                        <div class="flex items-center gap-2 mb-1.5">
                        ...
                        <p class="text-sm font-bold text-gray-800">{{ $agenda->unit ?? '-' }}</p>
                    </div>
                    <div class="p-4 rounded-2xl bg-gray-50 border border-gray-100">
                        <div class="flex items-center gap-2 mb-1.5">
                        ...
                        <p class="text-sm font-bold text-gray-800">{{ $agenda->meeting_chair }}</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div class="bg-white rounded-3xl border border-gray-100 p-6">
                ...
                @if($agenda->letter_file_path)
                    <a href="{{ Storage::url($agenda->letter_file_path) }}" target="_blank" class="inline-flex items-center gap-2 px-5 py-3 rounded-2xl bg-rose-50 text-rose-600 text-sm font-bold hover:bg-rose-100 active:scale-[0.98] transition-all duration-200 w-full justify-center">
            ...
            <div class="bg-white rounded-3xl border border-gray-100 p-6">
                ...
                @if($agenda->material_file_path)
                    <a href="{{ Storage::url($agenda->material_file_path) }}" target="_blank" class="inline-flex items-center gap-2 px-5 py-3 rounded-2xl bg-blue-50 text-blue-600 text-sm font-bold hover:bg-blue-100 active:scale-[0.98] transition-all duration-200 w-full justify-center">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178z"/><path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
                        Lihat Materi Rapat
                    </a>
                @else
                ...
                <table class="w-full">
                    <thead>
                        <tr class="border-b border-gray-100">
                            ...
                            <th class="px-6 py-3.5 text-left text-xs font-semibold text-gray-400 uppercase tracking-wider">NIP</th>
                            <th class="px-6 py-3.5 text-left text-xs font-semibold text-gray-400 uppercase tracking-wider">Jabatan</th>
                            <th class="px-6 py-3.5 text-left text-xs font-semibold text-gray-400 uppercase tracking-wider">Organisasi</th>
                        ...
                        @foreach($agenda->employees as $index => $employee)
                            <tr class="hover:bg-gray-50/50 transition-colors">
                                ...
                                <td class="px-6 py-3.5">
                                    <div class="flex items-center gap-3">
                                        <div class="w-7 h-7 rounded-lg {{ $employee->pivot->signature_image_path ? 'bg-primary-50' : 'bg-gray-100' }} flex items-center justify-center text-[10px] font-bold {{ $employee->pivot->signature_image_path ? 'text-primary' : 'text-gray-400' }}">{{ strtoupper(substr($employee->full_name, 0, 1)) }}</div>
                                ...
                                <td class="px-6 py-3.5 text-sm text-gray-500">{{ $employee->organization }}</td>
                                <td class="px-6 py-3.5">
                                ...
                                <td class="px-6 py-3.5 text-center">
                                    @if($employee->pivot->signature_image_path)
                                        ...
                                        >
                                            <img src="{{ Storage::url($employee->pivot->signature_image_path) }}" alt="TTD {{ $employee->full_name }}" class="w-16 h-10 object-contain bg-white group-hover:scale-105 transition-transform">
                                        </button>
                                    @else
                    ...
                    <div class="columns-2 md:columns-3 gap-4 space-y-4">
                        @foreach($agenda->images as $imgIndex => $image)
                            <div class="break-inside-avoid">
                                ...
                                >
                                    <img src="{{ Storage::url($image->image_path) }}" alt="Dokumentasi {{ $imgIndex + 1 }}" class="w-full rounded-2xl object-cover transition-transform duration-300 group-hover:scale-105" loading="lazy">
                                    <div class="absolute inset-0 bg-black/0 group-hover:bg-black/20 transition-colors duration-300 rounded-2xl flex items-center justify-center">
        ...
        <div
            x-show="showSignatureModal" x-cloak
        ...
        >
            ...
            >
                <div class="px-6 pt-6 pb-4 border-b border-gray-100 flex items-center justify-between">
                    ...
                    <button @click="showSignatureModal = false" class="w-8 h-8 rounded-xl bg-gray-100 hover:bg-gray-200 flex items-center justify-center transition-colors">
                ...
                <div class="p-6">
                    <div class="border-2 border-dashed border-gray-200 rounded-2xl p-4 bg-gray-50/50">
                        <img :src="signatureUrl" :alt="'Tanda tangan ' + signatureName" class="w-full h-48 object-contain">
                    </div>
                </div>
                <div class="px-6 pb-6">
                    <button @click="showSignatureModal = false" class="w-full px-4 py-2.5 rounded-2xl bg-gray-100 text-gray-700 text-sm font-semibold hover:bg-gray-200 active:scale-[0.98] transition-all duration-200">Tutup</button>
                </div>
            </div>
        </div>
        </template>
        ...
        >
            <button @click="showImageModal = false" class="absolute top-4 right-4 w-10 h-10 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center transition-colors">
            ...
            <img :src="imageUrl" alt="Dokumentasi" class="max-w-full max-h-[85vh] rounded-2xl object-contain shadow-2xl">
        </div>
        </template>
    </div>
</x-app-layout>
