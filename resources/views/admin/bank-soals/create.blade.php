        ...
        <form action="{{ route('admin.bank-soals.store') }}" method="POST" class="space-y-6">
            ...
            <div class="bg-white rounded-3xl border border-gray-100 overflow-hidden">
                <div class="px-8 py-6 border-b border-gray-100">
                    ...
                    <p class="text-sm text-gray-400 mt-0.5">Upload file CSV, XLSX, atau XLS. Kolom wajib: <span class="font-mono text-xs text-gray-500">question_text, option_a, option_b, option_c, correct_option</span> — opsional: <span class="font-mono text-xs text-gray-500">option_d, option_e</span></p>
                </div>
                <div class="p-8 space-y-5">
                    ...
                    {{-- Soal yang sudah dikonfirmasi --}}
                    <template x-if="questions.length > 0 && importPreview.length === 0">
                        <div class="space-y-3">
                            <div class="flex items-center justify-between">
                                <div class="flex items-center gap-2">
                                    <svg class="w-4 h-4 text-green-500 shrink-0" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                                    <p class="text-sm font-semibold text-gray-700"><span x-text="questions.length"></span> soal siap disimpan</p>
                                </div>
                                <button type="button" @click="questions = []; importFileName = ''" class="text-xs text-gray-400 hover:text-rose-500 transition-colors">Hapus semua</button>
                            </div>
                            <div class="overflow-x-auto rounded-2xl border border-gray-200">
                                <table class="w-full text-xs">
                                    <thead class="bg-gray-50 text-gray-500 font-semibold">
            ...
            <div class="flex items-center gap-3">
                <button type="submit" :disabled="questions.length === 0" class="px-6 py-3 rounded-2xl bg-primary text-white text-sm font-bold shadow-md shadow-primary/20 hover:bg-primary-700 hover:shadow-lg active:scale-[0.98] transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed">Simpan</button>
