<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-2 text-sm">
            ...
            <span class="font-semibold text-gray-700">Ubah Ruangan</span>
        </div>
    </x-slot>
    ...
    <div class="max-w-3xl">
        <div class="bg-white rounded-3xl border border-gray-100 overflow-hidden">
            <div class="px-8 py-6 border-b border-gray-100">
                <h3 class="text-lg font-bold text-gray-900">Ubah Ruangan</h3>
                <p class="text-sm text-gray-400 mt-0.5">Perbarui informasi ruangan</p>
            </div>
            <div class="p-8">
                <form action="{{ route('admin.rooms.update', $room) }}" method="POST" class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="room_name" id="room_name" value="{{ old('room_name', $room->room_name) }}" class="block w-full px-4 py-3 rounded-2xl border border-gray-200 bg-gray-50/50 text-sm text-gray-900 placeholder-gray-400 transition duration-200 focus:bg-white focus:border-primary focus:ring-2 focus:ring-primary/20 focus:outline-none" required>
                        @error('room_name') <p class="text-rose-500 text-xs font-medium mt-1.5">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        <label for="description" class="block text-sm font-semibold text-gray-700 mb-2">Deskripsi</label>
                        <textarea name="description" id="description" rows="3" class="block w-full px-4 py-3 rounded-2xl border border-gray-200 bg-gray-50/50 text-sm text-gray-900 placeholder-gray-400 transition duration-200 focus:bg-white focus:border-primary focus:ring-2 focus:ring-primary/20 focus:outline-none">{{ old('description', $room->description) }}</textarea>
