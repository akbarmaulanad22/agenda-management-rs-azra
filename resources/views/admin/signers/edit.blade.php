<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-2 text-sm">
            ...
            <span class="font-semibold text-gray-700">Ubah Penandatangan</span>
        </div>
    </x-slot>
    ...
    <div class="max-w-2xl">
        <div class="bg-white rounded-3xl border border-gray-100 overflow-hidden">
            <div class="px-8 py-6 border-b border-gray-100">
                <h3 class="text-lg font-bold text-gray-900">Ubah Data Penandatangan</h3>
                <p class="text-sm text-gray-400 mt-0.5">Perbarui informasi penandatangan surat</p>
            </div>
            <div class="p-8">
                <form action="{{ route('admin.signers.update', $signer) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
                    @csrf @method('PUT')
                    <div>
                        <label for="name" class="block text-sm font-semibold text-gray-700 mb-2">Nama Lengkap</label>
                        <input type="text" name="name" id="name" value="{{ old('name', $signer->name) }}" class="block w-full px-4 py-3 rounded-2xl border border-gray-200 bg-gray-50/50 text-sm text-gray-900 placeholder-gray-400 transition duration-200 focus:bg-white focus:border-primary focus:ring-2 focus:ring-primary/20 focus:outline-none" required>
                    ...
                    <div>
                        ...
                        <input type="text" name="position" id="position" value="{{ old('position', $signer->position) }}" class="block w-full px-4 py-3 rounded-2xl border border-gray-200 bg-gray-50/50 text-sm text-gray-900 placeholder-gray-400 transition duration-200 focus:bg-white focus:border-primary focus:ring-2 focus:ring-primary/20 focus:outline-none" required>
                    ...
                    <div>
                        ...
                        @if($signer->signature_path)
                            <div class="mb-3 p-3 rounded-2xl bg-gray-50 border border-gray-100 inline-block">
                                <img src="{{ Storage::url($signer->signature_path) }}" alt="Tanda tangan saat ini" class="h-16 object-contain">
                                <p class="text-[11px] text-gray-400 mt-1.5 font-medium">Tanda tangan saat ini</p>
                            </div>
                        @endif
                        <p class="text-xs text-gray-400 mb-2">Kosongkan jika tidak ingin mengubah tanda tangan</p>
