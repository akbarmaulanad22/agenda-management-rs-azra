<x-app-layout>
    <x-slot name="header">
        <div>
            <h2 class="text-xl font-extrabold text-gray-900 tracking-tight">Unit</h2>
            <p class="text-sm text-gray-400 mt-0.5">Kelola data unit organisasi</p>
        </div>
    </x-slot>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div class="flex items-center gap-2">
            ...
            <span class="text-sm font-semibold text-gray-500">{{ $units->total() }} unit</span>
        </div>
        <a href="{{ route('admin.units.create') }}" class="inline-flex items-center gap-2 px-5 py-2.5 rounded-2xl bg-primary text-white text-sm font-bold shadow-md shadow-primary/20 hover:bg-primary-700 hover:shadow-lg active:scale-[0.98] transition-all duration-200">
                        ...
                        <tr class="group hover:bg-gray-50/50 transition-colors">
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-1">
                                    <a href="{{ route('admin.units.edit', $unit) }}" class="p-2 rounded-xl hover:bg-primary-50 text-gray-400 hover:text-primary transition-colors" title="Ubah">
                                    ...
                                    <form action="{{ route('admin.units.destroy', $unit) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus?')">
                                        @csrf @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="2" class="px-6 py-12 text-center">
                                ...
                                <p class="text-sm text-gray-400">Belum ada data unit</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
