        ...
        <aside :class="mobileSidebar ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'"
            ...
            <nav class="flex-1 overflow-y-auto px-3 py-6 space-y-1.5">
                ...
                <a href="{{ route('admin.participants.index') }}"
                    class="group flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all duration-200
                              {{ request()->routeIs('admin.participants.*') ? 'bg-white/15 text-white shadow-lg shadow-black/10' : 'text-white/70 hover:bg-white/10 hover:text-white' }}">
                    <div
                        class="w-9 h-9 rounded-xl {{ request()->routeIs('admin.participants.*') ? 'bg-white/20' : 'bg-white/5 group-hover:bg-white/10' }} flex items-center justify-center flex-shrink-0 transition-colors">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round"
                                d="M15 19.128a9.38 9.38 0 002.625.372 9.337 9.337 0 004.121-.952 4.125 4.125 0 00-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 018.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0111.964-3.07M12 6.375a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0zm8.25 2.25a2.625 2.625 0 11-5.25 0 2.625 2.625 0 015.25 0z" />
                        </svg>
                    </div>
                    <span x-show="sidebarOpen" class="text-sm font-semibold">Peserta</span>
                </a>

                {{-- Penandatangan --}}
                <a href="{{ route('admin.signers.index') }}"
                    class="group flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all duration-200
                              {{ request()->routeIs('admin.signers.*') ? 'bg-white/15 text-white shadow-lg shadow-black/10' : 'text-white/70 hover:bg-white/10 hover:text-white' }}">
                    <div
                        class="w-9 h-9 rounded-xl {{ request()->routeIs('admin.signers.*') ? 'bg-white/20' : 'bg-white/5 group-hover:bg-white/10' }} flex items-center justify-center flex-shrink-0 transition-colors">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round"
                                d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" />
                        </svg>
                    </div>
                    <span x-show="sidebarOpen" class="text-sm font-semibold">Penandatangan</span>
                </a>

                {{-- Template --}}
                <a href="{{ route('admin.templates.index') }}"
                    class="group flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all duration-200
                              {{ request()->routeIs('admin.templates.*') ? 'bg-white/15 text-white shadow-lg shadow-black/10' : 'text-white/70 hover:bg-white/10 hover:text-white' }}">
                    <div
                        class="w-9 h-9 rounded-xl {{ request()->routeIs('admin.templates.*') ? 'bg-white/20' : 'bg-white/5 group-hover:bg-white/10' }} flex items-center justify-center flex-shrink-0 transition-colors">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round"
                                d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z" />
                        </svg>
                    </div>
                    <span x-show="sidebarOpen" class="text-sm font-semibold">Template Surat</span>
                </a>
