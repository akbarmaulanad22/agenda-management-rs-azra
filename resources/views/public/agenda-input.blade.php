...
<body class="bg-gray-50 min-h-screen font-sans">
    <div x-data="photoUploader()" class="max-w-2xl mx-auto pb-8">
        {{-- Header --}}
            ...
            <div class="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
                        ...
                        <div class="grid grid-cols-3 gap-2">
                            <template x-for="(img, index) in imagePreviews" :key="index">
                                <div class="relative">
                                    ...
                                    <button type="button" @click="removeImage(index)" class="absolute top-1 right-1 w-5 h-5 bg-red-500 text-white rounded-full flex items-center justify-center shadow-md hover:bg-red-600 transition">
                                        <svg class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="3"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/></svg>
                                    </button>
                                </div>
                            </template>
                        </div>
                        <p class="text-xs text-gray-400 text-center" x-text="imagePreviews.length + ' foto dipilih'"></p>
                        <button type="button" @click="uploadAll()" :disabled="uploading" class="w-full px-4 py-2.5 rounded-xl bg-primary text-white text-sm font-semibold shadow-md shadow-primary/20 hover:bg-primary-700 active:scale-[0.98] transition-all disabled:opacity-50">
                ...
                <template x-if="imagePreviews.length === 0">
                    <div class="border-2 border-dashed border-gray-200 rounded-xl p-6 text-center">
                        ...
                        <p class="text-sm text-gray-500 font-medium">Ambil atau pilih foto</p>
                        <p class="text-xs text-gray-400 mt-1">Gunakan tombol di atas, maks. 3MB per foto</p>
                    </div>
                </template>
            </div>
